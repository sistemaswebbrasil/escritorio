...
const rows: Ref<Role[] | undefined> = ref([]);
...
const update = async () => {
  try {
    await updateRole(Number(editedItem.value.id), editedItem.value);
    close();
    loadData();
  } catch (error) {}
};
              ...
              <v-card>
                ...
                <v-card-actions class="pa-4">
                  ...
                  <v-btn
                    color="secondary"
                    :disabled="
                      editedItem.name == '' || editedItem.description == ''
                    "
                    variant="flat"
                    @click="save"
                    >Save</v-btn
